...
const StandardTokenMock = artifacts.require('RemeCoin');
...
contract('StandardToken', function ([_, owner, recipient, anotherAccount, feeBeneficiary]) {
  ...
  beforeEach(async function () {
    this.token = await StandardTokenMock.new(FEE, feeBeneficiary, FEE_THRESHOLD, { from: owner });
  });
  ...
  describe('transfer', function () {
    describe('when the recipient is not the zero address', function () {
      ...
      describe('when the sender does not have enough balance', function () {
        const amount = new BigNumber(TOTAL_SUPPLY).add(1);
